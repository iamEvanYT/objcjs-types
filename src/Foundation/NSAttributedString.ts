// AUTO-GENERATED by objcjs-types generator — DO NOT EDIT
import type { NobjcObject } from "objc-js";
import type { _NSData } from "./NSData.js";
import type { _NSDictionary } from "./NSDictionary.js";
import type { _NSLocale } from "./NSLocale.js";
import type { _NSObject } from "./NSObject.js";
import type { _NSString } from "./NSString.js";
import type { _NSURL } from "./NSURL.js";
import type { NSRange } from "../structs.js";

export declare class _NSAttributedString extends _NSObject {
  static alloc(): _NSAttributedString;
  static new(): _NSAttributedString;
  init(): _NSAttributedString;
  // Class methods
  static localizedAttributedStringWithFormat$(format: _NSAttributedString): _NSAttributedString;
  static localizedAttributedStringWithFormat$options$(format: _NSAttributedString, options: number): _NSAttributedString;
  static localizedAttributedStringWithFormat$context$(format: _NSAttributedString, context: _NSDictionary): _NSAttributedString;
  static localizedAttributedStringWithFormat$options$context$(format: _NSAttributedString, options: number, context: _NSDictionary): _NSAttributedString;

  // Instance methods
  attributesAtIndex$effectiveRange$(location: number, range: number | null): _NSDictionary;
  attribute$atIndex$effectiveRange$(attrName: number, location: number, range: number | null): NobjcObject;
  attributedSubstringFromRange$(range: NSRange): _NSAttributedString;
  attributesAtIndex$longestEffectiveRange$inRange$(location: number, range: number | null, rangeLimit: NSRange): _NSDictionary;
  attribute$atIndex$longestEffectiveRange$inRange$(attrName: number, location: number, range: number | null, rangeLimit: NSRange): NobjcObject;
  isEqualToAttributedString$(other: _NSAttributedString): boolean;
  initWithString$(str: _NSString): _NSAttributedString;
  initWithString$attributes$(str: _NSString, attrs: _NSDictionary | null): _NSAttributedString;
  initWithAttributedString$(attrStr: _NSAttributedString): _NSAttributedString;
  enumerateAttributesInRange$options$usingBlock$(enumerationRange: NSRange, opts: number, block: (...args: any[]) => any): void;
  enumerateAttribute$inRange$options$usingBlock$(attrName: number, enumerationRange: NSRange, opts: number, block: (...args: any[]) => any | null): void;
  initWithContentsOfMarkdownFileAtURL$options$baseURL$error$(markdownFile: _NSURL, options: NobjcObject, baseURL: _NSURL | null, error: NobjcObject): _NSAttributedString | null;
  initWithMarkdown$options$baseURL$error$(markdown: _NSData, options: NobjcObject, baseURL: _NSURL | null, error: NobjcObject): _NSAttributedString | null;
  initWithMarkdownString$options$baseURL$error$(markdownString: _NSString, options: NobjcObject, baseURL: _NSURL | null, error: NobjcObject): _NSAttributedString | null;
  initWithFormat$options$locale$(format: _NSAttributedString, options: number, locale: _NSLocale | null): _NSAttributedString;
  initWithFormat$options$locale$arguments$(format: _NSAttributedString, options: number, locale: _NSLocale | null, arguments: NobjcObject): _NSAttributedString;
  initWithFormat$options$locale$context$(format: _NSAttributedString, options: number, locale: _NSLocale | null, context: _NSDictionary): _NSAttributedString;
  initWithFormat$options$locale$context$arguments$(format: _NSAttributedString, options: number, locale: _NSLocale | null, context: _NSDictionary, arguments: NobjcObject): _NSAttributedString;
  attributedStringByInflectingString(): _NSAttributedString;

  // Properties
  string(): _NSString;
  length(): number;
}
