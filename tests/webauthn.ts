...
const challenge = NSData.dataWithBytes$length$(challengeBuffer, challengeBuffer.length);
...
const platformKeyRequest = platformProvider.createCredentialRegistrationRequestWithChallenge$name$userID$(
  challenge,
  name,
  userID
);
...
platformKeyRequest.setRequestStyle$(ASAuthorizationPlatformPublicKeyCredentialRegistrationRequestStyle.Conditional);
